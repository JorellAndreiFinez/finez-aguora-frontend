        ...
        <aside class="fixed inset-y-0 left-0 z-30 flex w-[223px] flex-col border-r border-[#DDE5F0] bg-[#F8FAFC]">
            <div class="flex h-[64px] items-center justify-between px-3">
                <a href="{{ route('dashboard') }}" class="flex items-center gap-2.5">
                    <span
                        class="flex size-[38px] items-center justify-center rounded-md border border-[#DDE5F0] bg-white">
            ...
            <nav class="mt-2 space-y-1 px-3 text-[15px] font-semibold text-[#607089]">
                <a href="{{ route('dashboard') }}"
                    ...
                    <svg viewBox="0 0 24 24">
                        <path d="M4 19V5m0 14h16M7 15l4-4 3 3 5-7" />
                    </svg>
                    Dashboard
                </a>
                <a href="{{ route('orders.index') }}"
                    class="nav-link {{ request()->routeIs('orders.*') ? 'is-active' : '' }}">
                    <svg viewBox="0 0 24 24">
                        <path d="M4 7h16M4 12h16M4 17h16M8 7v10" />
                    </svg>
                    Order List
                </a>
                ...
                <a href="{{ route('products.index') }}"
                    class="nav-link {{ request()->routeIs('products.*') || request()->routeIs('categories.*') ? 'is-active' : '' }}">
                    <svg viewBox="0 0 24 24">
                ...
                <div class="space-y-1 pl-[52px] text-[15px]">
                    <a href="{{ route('products.index') }}"
                        class="block rounded px-3 py-2 font-semibold {{ request()->routeIs('products.*') ? 'bg-[#DCE6F8] text-[#0057E8]' : 'text-[#607089]' }}">Product
                        List</a>
                    <a href="{{ route('categories.index') }}"
                        class="block rounded px-3 py-2 font-semibold {{ request()->routeIs('categories.*') ? 'bg-[#DCE6F8] text-[#0057E8]' : 'text-[#607089]' }}">Categories</a>
                </div>
                ...
                <div class="space-y-1 pl-[52px] text-[15px]">
                    <a href="{{ route('reports.sales') }}"
                        class="block rounded px-3 py-2 font-semibold {{ request()->routeIs('reports.sales') ? 'bg-[#DCE6F8] text-[#0057E8]' : 'text-[#607089]' }}">Sales
                        Report</a>
                    <a href="{{ route('reports.inventory') }}"
                        class="block rounded px-3 py-2 font-semibold {{ request()->routeIs('reports.inventory') ? 'bg-[#DCE6F8] text-[#0057E8]' : 'text-[#607089]' }}">Inventory
                        Report</a>
                </div>
            </nav>
            ...
            <div class="mt-auto flex items-center gap-3 px-5 pb-5">
                <div
                    class="grid size-8 place-items-center overflow-hidden rounded-full bg-gradient-to-br from-[#D9B36C] via-[#F2D2B6] to-[#091E42] text-xs font-bold text-white">
                    JS</div>
                <div class="min-w-0">
